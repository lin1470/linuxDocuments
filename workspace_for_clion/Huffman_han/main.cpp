...
void haffman(int weight[],char ch[],int n,HafNode haffTree[])  //生成哈夫曼树的函数
{
    ...
    fprintf(fp,"%d",n);
    for (i=0;i<n;i++)
        fprintf(fp,"%c %d %d %d",haffTree[i].ch,haffTree[i].parent,haffTree[i].lchild,haffTree[i].rchild);
    for (i=n;i<2*n-1;i++)
        fprintf(fp,"%d %d %d",haffTree[i].parent,haffTree[i].lchild,haffTree[i].rchild);
    fclose(fp);
}
...
void Init(int weight[],char ch[])       //初始化操作，生成哈夫曼树及哈夫曼编码
{
    ...
    int i,j,n;
    char ch1,wj[15];
    printf("现在进行初始化操作。。。请选择:A.键盘输入            B.文件输入");
    scanf("%c",&ch1);
    if (ch1=='A')
    {
        printf("请输入字符集大小n:");
...
void Caozuo_C()     //哈夫曼编码过程的函数，用于将文件编码
{
    ...
    fclose(fp);
    printf("现在进行编码操作。。。请选择:A.键盘输入            B.文件输入");
    scanf("%s",&c);
    if (c=='A')
    {
...
void Caozuo_D()               //译码操作
{
    ...
    HafNode *myHaffTree=(HafNode *)malloc(sizeof (HafNode)*(2*n+1));
    for (i=0;i<n;i++)
        fscanf(fp,"%s %d %d %d",&myHaffTree[i].ch,&myHaffTree[i].parent,&myHaffTree[i].lchild,&myHaffTree[i].rchild);
    for (i=n;i<2*n-1;i++)
...
void PrintTree(HafNode *huf,int n,int p,FILE *fp)
{
    ...
    {
        printf("---");
        printf("%c",huf[n].ch);            //如果此结点为叶子结点，则将此结点输出；
        fprintf(fp,"---%c",huf[n].ch);
    }
    else
    {
        printf("@");                     //如果此结点为非叶子结点，则输出"@";
        fprintf(fp,"@");
    }
...
void Caozuo_T()            //打印哈夫曼树的操作
{
    ...
    fclose(fp);
    printf("打印哈夫曼树过程完成!同时已将结果存入%s中.",ch1);
}
